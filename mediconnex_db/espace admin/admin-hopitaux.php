...
<head>
    ...
    <title>Gestion des hôpitaux - MédiConnex</title>
...
<body>
    <div class="gestion-wrapper">
        ...
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li class="active"><a href="admin-hopitaux.php"><i class="fa-solid fa-hospital"></i> Hôpitaux</a></li>
        ...
        <div class="main-content">
            <div class="page-header">
                ...
                <a href="admin-ajout-hopital.php" class="btn-add">
                    <i class="fa-solid fa-plus"></i> Ajouter un hôpital
                </a>
            </div>

            <div class="table-container">
                <table id="hopitauxTable">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Nom hôpital</th>
    ...
    <div id="modalModif" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2><i class="fa-solid fa-pen"></i> Modifier hôpital</h2>
    ...
    <script>
        ...
        let hopitaux = [
            { id: 'H001', nom: 'CHU Mustapha', wilaya: 'Alger (16)', adresse: 'Alger Centre', tel: '023 45 67 89' },
            { id: 'H002', nom: 'CHU Oran', wilaya: 'Oran (31)', adresse: 'Oran', tel: '041 34 56 78' },
            { id: 'H003', nom: 'CHU Constantine', wilaya: 'Constantine (25)', adresse: 'Constantine', tel: '031 23 45 67' },
            { id: 'H004', nom: 'CHU Annaba', wilaya: 'Annaba (23)', adresse: 'Annaba', tel: '038 45 67 89' }
        ];

        let ligneEnCours = null;

        function afficherHopitaux() {
            const tbody = document.getElementById('hopitauxBody');
            tbody.innerHTML = '';

            hopitaux.forEach(h => {
                const ligne = document.createElement('tr');
                ligne.setAttribute('data-id', h.id);
                ligne.setAttribute('data-nom', h.nom);
                ligne.setAttribute('data-wilaya', h.wilaya);
                ligne.setAttribute('data-adresse', h.adresse);
                ligne.setAttribute('data-tel', h.tel);
                ligne.innerHTML = `
                    <td>${h.id}</td>
                    <td>${h.nom}</td>
                    <td>${h.wilaya}</td>
                    ...
                    <td class="action-icons">
                        ...
                        <i class="fa-solid fa-trash" title="Supprimer" onclick="supprimerHopital(this)"></i>
                    </td>
                `;
        ...
        // Supprimer
        window.supprimerHopital = function(icone) {
            if (confirm('🗑️ Supprimer cet hôpital ?')) {
                const ligne = icone.closest('tr');
                const id = ligne.getAttribute('data-id');
                hopitaux = hopitaux.filter(h => h.id !== id);
                afficherHopitaux();
            }
        };
        ...
        window.ouvrirModal = function(icone) {
            ...
            document.getElementById('editAdresse').value = ligneEnCours.getAttribute('data-adresse');
            document.getElementById('editTel').value = ligneEnCours.getAttribute('data-tel');
        ...
        window.enregistrerModification = function() {
            ...
            const newAdresse = document.getElementById('editAdresse').value;
            const newTel = document.getElementById('editTel').value;

            const hopital = hopitaux.find(h => h.id === id);
            if (hopital) {
                hopital.nom = newNom;
                hopital.wilaya = newWilaya;
                hopital.adresse = newAdresse;
                hopital.tel = newTel;
            }

            ligneEnCours.setAttribute('data-nom', newNom);
            ligneEnCours.setAttribute('data-wilaya', newWilaya);
            ligneEnCours.setAttribute('data-adresse', newAdresse);
            ligneEnCours.setAttribute('data-tel', newTel);
            ligneEnCours.cells[1].innerText = newNom;
            ligneEnCours.cells[2].innerText = newWilaya;
            ligneEnCours.cells[3].innerText = newAdresse;
            ligneEnCours.cells[4].innerText = newTel;
